...
function useSponsorImageSrc(imagePath: string): string {
  ...
  useEffect(() => {
    let cancelled = false
    const run = async () => {
      if (!imagePath.trim()) { setSrc(''); return }
      if (imagePath.startsWith('file:') || imagePath.startsWith('http')) {
        setSrc(imagePath)
        return
      }
      const api = window.electronAPI
      if (!api?.getImagePath) { setSrc(''); return }
      try {
...
function FullscreenCard({ name, imagePath }: {
  name: string
  imagePath: string
}) {
  ...
  return (
    <>
      ...
      <div
        ...
        style={{
          background: 'radial-gradient(ellipse 60% 50% at 50% 45%, rgba(255,255,255,0.04) 0%, transparent 65%)',
        }}
      />
      ...
      <motion.h2
        ...
        style={{
          textShadow: '0 0 20px rgba(255,255,255,0.15), 0 2px 4px rgba(0,0,0,0.5)',
        }}
