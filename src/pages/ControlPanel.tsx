  ...
  return (
    <div ref={setNodeRef} style={style} className="group/sortable min-w-0">
      ...
      <PlayerCard
        ...
        onClick={onClick}
        capNumber={capNumber}
        usedCapNumbers={usedCapNumbers}
        onCapNumberChange={onCapNumberChange}
      />
    </div>
  );
}
...
export function ControlPanel() {
  ...
  const [presenting, setPresenting] = useState<PresentingState>({
    kind: "idle",
  });
  ...
  const [showCapNumbers, setShowCapNumbers] = useState(false);
  ...
  const clubById = useMemo(
    () => (data ? clubMapFromData(data) : new Map<string, Club>()),
    [data],
  );
  ...
  useEffect(() => {
    if (!api) {
      setLoadError(
        "Electron API is not available. Open this screen from the desktop app.",
      );
      return;
    }
    void api
  ...
  const showMatchup = useCallback(() => {
    ...
    api?.sendCommand({
      ...
      homeClubId,
      awayClubId,
      matchLabel: matchLabel.trim() || "Match",
    });
  ...
  const filteredPlayers = useMemo(() => {
    if (!data) return [];
    return selectedClubIds.size === 0
      ? data.players
      : data.players.filter((p) => selectedClubIds.has(p.clubId));
  }, [data, selectedClubIds]);
  ...
  const handleDragEndWithScroll = useCallback(
    (clubId: string, event: DragEndEvent) => {
  ...
  const setCapNumber = useCallback(
    (clubId: string, playerId: string, num: number | undefined) => {
      if (!data) return;
      const prev = data.capNumbers ?? {};
      const clubCaps = { ...(prev[clubId] ?? {}) };
      if (num === undefined) {
        delete clubCaps[playerId];
      } else {
        clubCaps[playerId] = num;
      }
      const newCapNumbers = { ...prev, [clubId]: clubCaps };
      const newData = { ...data, capNumbers: newCapNumbers };
      setData(newData);
      window.electronAPI?.saveData(newData);
    },
    [data],
    ...
    <div className="flex h-full min-h-0 w-full flex-col bg-base text-txt-primary">
      ...
      <header className="shrink-0 border-b border-border-subtle bg-surface/60 backdrop-blur-sm">
        <div className="flex items-center gap-5 px-10 py-4">
          ...
          >
            ...
            />
            <span className={isLive ? "text-emerald-300" : "text-txt-tertiary"}>
              LIVE
            </span>
          </div>
        ...
        {data && (
          <div className="flex flex-col gap-3 px-10 py-4">
            ...
            <div className="flex items-center gap-2">
              <span className="w-16 shrink-0 text-[10px] font-bold uppercase tracking-wider text-txt-tertiary">
                Match
              </span>
              <select
                value={homeClubId}
            ...
            {/* Row 4 — Cap numbers toggle */}
            <div className="flex items-center gap-2">
              <span className="w-16 shrink-0 text-[10px] font-bold uppercase tracking-wider text-txt-tertiary">
                Caps
              </span>
              <label className="flex cursor-pointer items-center gap-2 text-xs text-txt-secondary">
                <input
                  type="checkbox"
                  checked={showCapNumbers}
                  onChange={(e) => setShowCapNumbers(e.target.checked)}
                  className="size-3.5 rounded border-border-default bg-raised accent-accent"
                />
                Show cap number selectors
              </label>
            </div>
          </div>
        )}
      ...
      <main ref={scrollRef} className="min-h-0 flex-1 overflow-y-auto">
        <div className="px-10 py-5">
          ...
          {!data && !loadError && (
            <div className="flex h-48 items-center justify-center text-txt-tertiary">
              Loading roster…
            </div>
          )}
                ...
                >
                  {!homeClubId || !awayClubId ? (
                    <p className="py-6 text-center text-sm text-txt-tertiary">
                      Select both clubs in the match setup above
                    </p>
                  ) : (
                    (() => {
                      ...
                      const isHomeClubActive =
                        presenting.kind === "club" &&
                        presenting.clubId === homeClub.id;
                      const isAwayClubActive =
                        presenting.kind === "club" &&
                        presenting.clubId === awayClub.id;
                      return (
                        <div className="flex flex-col gap-4">
                            ...
                            <div className="flex items-center justify-center gap-8">
                              <div className="flex flex-col items-center gap-2">
                                <div className="flex size-16 items-center justify-center overflow-hidden rounded-xl">
                                  <ElectronImage
                                    filename={homeClub.logoPath}
                                    alt={homeClub.name}
                                    className="size-full"
                                    fit="contain"
                                  />
                                </div>
                                <p className="max-w-[120px] truncate text-center text-sm font-semibold">
                                  {homeClub.name}
                                </p>
                              </div>
                              <span className="text-lg font-black text-txt-tertiary">
                                VS
                              </span>
                              <div className="flex flex-col items-center gap-2">
                                <div className="flex size-16 items-center justify-center overflow-hidden rounded-xl">
                                  <ElectronImage
                                    filename={awayClub.logoPath}
                                    alt={awayClub.name}
                                    className="size-full"
                                    fit="contain"
                                  />
                          ...
                          <div className="grid grid-cols-2 gap-4">
                            {[homeClub, awayClub].map((club) => {
                              ...
                              const playerCount = data.players.filter(
                                (p) => p.clubId === club.id,
                              ).length;
                              return (
                                <div
              ...
              {tab === "players" && (
                ...
                >
                  {selectedClubIds.size === 0 ? (
                    <p className="py-6 text-center text-sm text-txt-tertiary">
                      Pick two clubs above to filter players
                    </p>
                  ) : filteredPlayers.length === 0 ? (
                    <p className="text-txt-tertiary">
                      No players for the selected clubs.
                    </p>
                  ) : (
                    <div className="flex flex-col gap-10">
                      {orderedGrouped.map(({ clubId, players }) => {
                        ...
                        const clubLive =
                          presenting.kind === "club" &&
                          club &&
                          presenting.clubId === club.id;
                        const itemIds = players.map((p) => p.id);
                        const clubCaps = data.capNumbers?.[clubId] ?? {};
                        const usedCaps = new Set(Object.values(clubCaps));
                        return (
                          <div key={clubId}>
                            <button
                              type="button"
                              ...
                              className={[
                                ...
                                clubLive
                                  ? "ring-1 ring-emerald-400/80 bg-raised"
                                  : "",
                              ].join(" ")}
                            >
                              {club ? (
                              ...
                              {clubLive && (
                                <span className="ml-auto text-[9px] font-bold uppercase tracking-wide text-emerald-400">
                                  Live
                                </span>
                              )}
                            </button>
                            <DndContext
                              ...
                              onDragEnd={(e) =>
                                handleDragEndWithScroll(clubId, e)
                              }
                                ...
                                >
                                  {players.map((player) => (
                                    <SortablePlayerCard
                                      ...
                                      club={club}
                                      isActive={
                                        presenting.kind === "player" &&
                                        presenting.playerId === player.id
                                      }
                                      onClick={() => showPlayer(player)}
                                      capNumber={clubCaps[player.id]}
                                      usedCapNumbers={
                                        showCapNumbers ? usedCaps : undefined
                                      }
                                      onCapNumberChange={
                                        showCapNumbers
                                          ? (num) =>
              ...
              {tab === "officials" && (
                ...
                >
                  {!allOfficialsSelected ? (
                    <p className="py-6 text-center text-sm text-txt-tertiary">
                      Select two referees and a delegate in the toolbar above to
                      present officials
                    </p>
                  ) : (
                    (() => {
                      const ref1 = data.officials.find(
                        (o) => o.id === referee1Id,
                      );
                      const ref2 = data.officials.find(
                        (o) => o.id === referee2Id,
                      );
                      const del = data.officials.find(
                        (o) => o.id === delegateId,
                      );
                        ...
                        >
                          ...
                          <div className="mb-4 flex justify-center gap-8">
                            <div className="flex flex-col items-center gap-1.5">
                              <div className="flex size-10 items-center justify-center rounded-full bg-amber-500/15 text-xs font-bold text-amber-400">
                                R
                              </div>
                              <p className="max-w-[100px] truncate text-center text-xs font-semibold">
                                {ref1?.name ?? "—"}
                              </p>
                              <p className="text-[9px] text-txt-tertiary">
                                Referee
                              </p>
                            </div>
                            <div className="flex flex-col items-center gap-1.5">
                          ...
                          <div className="flex flex-col items-center gap-1.5">
                            <div className="flex size-10 items-center justify-center rounded-full bg-cyan-500/15 text-xs font-bold text-cyan-400">
                              D
                            </div>
                            <p className="max-w-[140px] truncate text-center text-xs font-semibold">
                              {del?.name ?? "—"}
                            </p>
                            <p className="text-[9px] text-txt-tertiary">
                              Delegate
                  ...
                  {data.sponsorStacks.length === 0 ? (
                    ...
                    <ul className="grid grid-cols-2 gap-4">
                      {data.sponsorStacks.map((stack) => {
                        ...
                        const isActive =
                          presenting.kind === "sponsors" &&
                          presenting.stackId === stack.id;
                        return (
                          <motion.li
                            key={stack.id}
                            layout
                          ...
                          >
                            ...
                            <p className="mt-1 text-xs text-txt-tertiary">
                              {stack.sponsorIds.length} sponsor
                              {stack.sponsorIds.length === 1 ? "" : "s"} ·{" "}
                              <span className="font-mono text-txt-secondary">
                                {stack.animationVariant}
                              </span>
                            </p>
                            <AnimationThumbnail
                              variant={stack.animationVariant}
                              className="mt-3"
                            />
